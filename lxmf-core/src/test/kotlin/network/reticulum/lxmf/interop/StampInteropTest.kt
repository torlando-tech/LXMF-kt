...
class StampInteropTest : LXMFInteropTestBase() {
    ...
    inner class InvalidStampRejection {
        ...
        fun `wrong workblock (message hash) rejected by both implementations`() {
            ...
            // Use target_cost=8 so false-positive probability is ~1/256 (0.4%)
            // instead of target_cost=4 which gives ~1/16 (6.25%) — too flaky
            val targetCost = 8

            println("  Generating stamp for message A (target_cost=$targetCost)...")
            val stampForA = runBlocking { LXStamper.generateStamp(workblockA, targetCost) }.stamp!!
            ...
            val kotlinValid = LXStamper.isStampValid(stampForA, targetCost, workblockB)
